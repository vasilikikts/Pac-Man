...
## Uninformed and informed search problems.
...
def followAction(state, action, problem):
  for nextState1, action1, cost1 in problem.getNextStates(state):
    if action == action1: return nextState1
  return None
...
class GraphSearch(SearchProblem):
    ...
    def __init__(self, graph_text):
        ...
        r = re.match('initial_state:(.*)', lines[0])
        if r == None:
            print("Broken graph:")
            print('"""%s"""' % graph_text)
            raise Exception("GraphSearch graph specification initial_state not found or incorrect on line 0")
        self.initial_state = r.group(1).strip()
        ...
        for l in lines[2:]:
            ...
            cost = float(cost)
            self.orderedNextStateTuples.append((start, action, next_state, cost))
            all_states.add(start)
            all_states.add(next_state)
            if start not in self.nextStates:
                self.nextStates[start] = []
            self.nextStates[start].append((next_state, action, cost))
        for s in all_states:
    ...
    # Get all nextStates of a state
    def getNextStates(self, state):
        self.expanded_states.append(state)
        return list(self.nextStates[state])
    ...
    def getCostOfActions(self, actions):
        ...
        for a in actions:
            nextStates = self.nextStates[state]
            match = False
            for (next_state, action, cost) in nextStates:
                if a == action:
                    state = next_state
                    total_cost += cost
                    match = True
    ...
    # Return a list of all states on which 'getNextStates' was called
    def getExpandedStates(self):
        return self.expanded_states

    def __str__(self):
        print(self.nextStates)
        edges = ["%s %s %s %s" % t for t in self.orderedNextStateTuples]
        return \
"""initial_state: %s
goal_states: %s
%s""" % (self.initial_state, " ".join(self.goals), "\n".join(edges))
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, searchAgents):
        ...
        initial_state = pacman.GameState()
        initial_state.initialize(lay, 0)

        problemClass = getattr(searchAgents, self.searchProblemClassName)
        problemOptions = {}
        if self.costFn != None:
            problemOptions['costFn'] = self.costFn
        problem = problemClass(initial_state, **problemOptions)
...
class HeuristicTest(testClasses.TestCase):
    ...
    def checkHeuristic(self, heuristic, problem, state, solutionCost):
        ...
        for nState, action, stepCost in problem.getNextStates(state):
            h1 = heuristic(nState, problem)
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        h0 = searchAgents.cornersHeuristic(initial_state, problem)
        nStates = problem.getNextStates(initial_state)
        # cornerConsistencyA
        for nState in nStates:
            h1 = searchAgents.cornersHeuristic(nState[0], problem)
            if h0 - h1 > 1:
                grades.addMessage('FAIL: inconsistent heuristic')
                return False
        heuristic_cost = searchAgents.cornersHeuristic(initial_state, problem)
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        initial_state = pacman.GameState()
        initial_state.initialize(lay, 0)
        problem = searchAgents.CornersProblem(initial_state)
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem = searchAgents.CornersProblem(game_state)
        initial_state = problem.getInitialState()
        if searchAgents.cornersHeuristic(initial_state, problem) > true_cost:
            grades.addMessage('FAIL: Inadmissible heuristic')
            return False
...
class MultiagentTreeState(object):
    ...
    def generateNextState(self, agentIndex, action):
        if VERBOSE:
            print("generateNextState(%s, %s, %s) -> %s" % (self.state, agentIndex,
                                                           action, self.problem.stateToNextStateMap[self.state][action]))
        nextState = self.problem.stateToNextStateMap[self.state][action]
        self.problem.generatedStates.add(nextState)
        return MultiagentTreeState(self.problem, nextState)
    ...
    def getAvailableActions(self, agentIndex=0):
        if VERBOSE:
            print("getAvailableActions(%s) -> %s" %
                  (self.state, self.problem.stateToActions[self.state]))
        # if len(self.problem.stateToActions[self.state]) == 0:
        #    print "WARNING: getAvailableActions called on leaf state %s" % (self.state,)
...
class MultiagentTreeProblem(object):
    def __init__(self, numAgents, initialState, winStates, loseStates, nextStates, evaluation):
        self.initialState = MultiagentTreeState(self, initialState)
        ...
        self.stateToNextStateMap = defaultdict(dict)
        self.stateToActions = defaultdict(list)
        for state, action, nextState in nextStates:
            self.stateToActions[state].append(action)
            self.stateToNextStateMap[state][action] = nextState

    def reset(self):
        self.generatedStates = set([self.initialState.state])


def parseTreeProblem(testDict):
    numAgents = int(testDict["num_agents"])
    initialState = testDict["initial_state"]
    ...
    for line in testDict["nextStates"].split('\n'):
        tokens = line.split()
        if len(tokens) == 3:
            state, action, nextState = tokens
            nextStates.append((state, action, nextState))
        else:
            raise Exception("[parseTree] Bad nextState line: |%s|" % (line,))

    return MultiagentTreeProblem(numAgents, initialState, winStates, loseStates, nextStates, evaluation)
...
class GraphGameTreeTest(testClasses.TestCase):
    ...
    def solveProblem(self, multiAgents):
        self.problem.reset()
        studentAgent = getattr(multiAgents, self.alg)(depth=self.depth)
        action = studentAgent.getAction(self.problem.initialState)
